...
CIISxpressRegSettings::CIISxpressRegSettings(void) : m_dwOpenFlags(0), m_hChangeNotificationThread(INVALID_HANDLE_VALUE)
{	
	m_dwLoadCookie = MakeLoadCookie();
}

CIISxpressRegSettings::~CIISxpressRegSettings(void)
{
	if (m_bDirty == true)
	{
		Save();
...
void CIISxpressRegSettings::Init(LPCTSTR pszRegPath, bool bLoad, bool bRegisterChangeHandler)
{
	m_bDirty = true;
...
#if _DEBUG
			::SetThreadName(nChangeNotificationThreadId, "ChangeNotificationProc");
#endif
		}
	}
}

DWORD WINAPI CIISxpressRegSettings::ChangeNotificationProc(void* pData)
{
	DWORD dwStatus = ((CIISxpressRegSettings*) pData)->ChangeNotificationProc();
...
{	
	...
	AutoHandle::AutoEventHandle RegChangeEvent(hRegChange);
...
bool CIISxpressRegSettings::GetEnabled(void)
{	
...
void CIISxpressRegSettings::SetEnabled(bool bEnabled, bool bSave)
{
...
bool CIISxpressRegSettings::GetDebugEnabled(void)
{
	// we don't need the interlock since the Set will do it for us
...
/*void CIISxpressRegSettings::SetDebugEnabled(bool bEnabled, bool bSave)
{
	CComCritSecLock<CComAutoCriticalSection> lock(m_csConfig);
...
DWORD CIISxpressRegSettings::GetLoggingLevel(void)
{	
	// we don't need the interlock since the Set will do it for us
...
bool CIISxpressRegSettings::SetLoggingLevel(DWORD dwLevel, bool bSave)
{
...
DWORD CIISxpressRegSettings::MakeLoadCookie()
{
