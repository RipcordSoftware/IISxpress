...
// CM: a new version based on CImage
class CImgDesc
{
public:    
	CImgDesc(CImage* pImage = NULL) : m_pImage(pImage)
	{
	}
...
public:
	...
	// CM: add a new CImage friendly version
    //static void SetMenuItemImage (UINT nID, HIMAGELIST hImgList, int nIndex);
	static void SetMenuItemImage (UINT nID, CImage* pImage);
