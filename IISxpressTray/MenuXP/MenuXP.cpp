...
bool CMenuItem::GetDisabled () const
{
	...
	// CM: correct the disabled handling logic
...
bool CMenuItem::Draw (CDC* pDC, LPCRECT pRect, bool bSelected, bool bMenuBar, bool bHotLight, bool bInactive, bool bNoAccel) const
{
    ...
    else
    {
        ...
        if ( GetCaption (sCaption) > 0 )
        {
			...
			if (bInactive == true)
			{
				crTextColor = ::GetSysColor (COLOR_3DSHADOW);
			}
			else if (GetDisabled() == true)
			{
				crTextColor = HLS_TRANSFORM (::GetSysColor (COLOR_3DFACE), -18, 0);				
			}
			else
			{
            ...
            else
            {
                ...
                if ( m_ImgDesc.m_pImage != NULL )
                {
                    bool bOver = !GetDisabled() && bSelected;

                    if ( GetDisabled() || (bSelected && !GetChecked()) )
                    {
						// CM: probably need a new version of this in the future						
                        //HICON hIcon = ImageList_ExtractIcon (NULL, m_ImgDesc.m_hImgList, m_ImgDesc.m_nIndex);
                        //pDC->DrawState (CPoint (pRect->left + ( bOver ? 4 : 3 ), rc.top + ( bOver ? 4 : 3 )),
                        //                CSize (IMGWIDTH, IMGHEIGHT), hIcon, DSS_MONO,
                        //                CBrush (bOver ? HLS_TRANSFORM (::GetSysColor (COLOR_HIGHLIGHT), +50, -66) : HLS_TRANSFORM (::GetSysColor (COLOR_3DFACE), -27, 0)));
                    ...
                    {
						// CM: draw the image
						m_ImgDesc.m_pImage->Draw(pDC->m_hDC, 
							pRect->left+3,
							rc.top+3);
                    }
                }
                ...
                {
                    // Draw the check mark
					// CM: Vista has slightly different positioning requirements
...
void CMenuXP::SetXPLookNFeel (CWnd* pWnd, HMENU hMenu, bool bXPLook, bool bMenuBar)
{
	...
	// CM: determine if it's a system menu based on the presence of SC_CLOSE
	UINT nState = ::GetMenuState(hMenu, SC_CLOSE, MF_BYCOMMAND);	
	if (nState != (UINT) -1)
		return;
...
///////////////////////////////////////////////////////////////////////////////
// CM: add new CImage based version
void CMenuXP::SetMenuItemImage (UINT nID, CImage* pImage)
{ 
	CImgDesc imgDesc (pImage);
    ms_Images.SetAt (nID, imgDesc);	
}
...
{
	// CM: don't handle the call unless we have a vaid item
	//if ( pDrawItemStruct->CtlType != ODT_MENU )
    if ( pDrawItemStruct->CtlType != ODT_MENU || pDrawItemStruct->itemID == -1)
    {
        return false;
    }
...
void DrawShadow (HDC hDCIn, HDC hDCOut, RECT& rc)
{
	...
	// CM: needed the second int x decl
    for ( int x = 0; x < SM_CXSHADOW; x++ )
    {
