// HelpAwareDialogBase.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CHelpAwareDialogBase, CDialog)

CHelpAwareDialogBase::CHelpAwareDialogBase(UINT nIDTemplate, CWnd* pParent /*=NULL*/)
	: CDialog(nIDTemplate, pParent)
{
}

CHelpAwareDialogBase::~CHelpAwareDialogBase()
{
}


BEGIN_MESSAGE_MAP(CHelpAwareDialogBase, CDialog)
	ON_WM_HELPINFO()
END_MESSAGE_MAP()

BOOL CHelpAwareDialogBase::OnHelpInfo(HELPINFO* pHelpInfo)
{
...
BOOL CHelpAwareDialogBase::PreTranslateMessage(MSG* pMsg)
{
...
BOOL CHelpAwareDialogBase::OnNotify(WPARAM wParam, LPARAM lParam, LRESULT* pResult)
{
	...
	else if (pNmHdr->code == TTN_NEEDTEXT && pNmHdr->hwndFrom == m_cContextHelp)
	{
		...
		{
			_tcscpy_s(m_szBuffer, _countof(m_szBuffer), sMsg);		
			pDispInfo->lpszText = m_szBuffer;
		}
	}
